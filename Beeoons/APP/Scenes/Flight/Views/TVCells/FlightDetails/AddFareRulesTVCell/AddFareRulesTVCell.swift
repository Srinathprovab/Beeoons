...
class AddFareRulesTVCell: TableViewCell, FareRulesTVCellDelegate {
    ...
    @IBAction func didTapOnFareRulesBtnAction(_ sender: Any) {
        ...
        if isToggled {
            ...
            dropimg.image = UIImage(named: "downarrow1")?.withRenderingMode(.alwaysOriginal).withTintColor(.NavBackColor)
            tvHeight.constant = CGFloat(fareRulesData1.count * 46)
            farerulesTV.reloadData()
        } else {
            dropimg.image = UIImage(named: "dropup1")?.withRenderingMode(.alwaysOriginal).withTintColor(.NavBackColor)
            tvHeight.constant = 0
            farerulesTV.reloadData()
        }
...
extension AddFareRulesTVCell:UITableViewDelegate,UITableViewDataSource {
    ...
    func setupTV() {
        ...
        farerulesTV.layer.borderColor = UIColor.CalenderRangeColor.cgColor
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let data = fareRulesData1[indexPath.row]
